...
import { PlayerStats, Archetype } from '../../lib/gameTypes';
  ...
  return (
    <div className="fixed top-0 left-0 w-full p-6 flex justify-between items-start pointer-events-none z-50">
      ...
      <div className="flex flex-col gap-4 pointer-events-auto">
        <div className="hud-border bg-hud-bg p-4 rounded-sm min-w-[250px] backdrop-blur-md">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="text-[9px] uppercase opacity-50 block text-hud-blue/50">Infection</span>
              <span className={`text-sm font-bold ${stats.infectionLevel > 50 ? 'text-red-500' : 'text-hud-blue'}`}>
                {stats.infectionLevel.toFixed(1)}%
              </span>
            </div>
            <div>
              <span className="text-[9px] uppercase opacity-50 block text-hud-blue/50">Confirmed Kills</span>
              <span className="text-sm font-bold text-hud-blue">{stats.kills}</span>
            </div>
          </div>
        </div>

        <div className="hud-border bg-hud-bg p-3 rounded-sm flex flex-col gap-3 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-hud-blue/10 border border-hud-blue/30">
              {stats.archetype === Archetype.POINTMAN && <Shield className="w-5 h-5 text-hud-blue" />}
              {stats.archetype === Archetype.MEDIC && <Zap className="w-5 h-5 text-hud-blue" />}
              {stats.archetype === Archetype.SHARPSHOOTER && <Crosshair className="w-5 h-5 text-hud-blue" />}
            </div>
            <div>
              <div className="text-[10px] uppercase opacity-50 text-hud-blue/50">Active Archetype</div>
              <div className="text-xs font-bold tracking-widest uppercase text-hud-blue">{stats.archetype}</div>
            </div>
          </div>
          ...
          <div className="flex gap-2 border-t border-hud-blue/10 pt-2">
            {Object.values(Archetype).map((type) => (
              <button 
                ...
                className={`px-2 py-1 text-[8px] uppercase tracking-widest border transition-all ${stats.archetype === type ? 'bg-hud-blue text-black border-hud-blue' : 'border-hud-blue/30 text-hud-blue/60 hover:border-hud-blue/60'}`}
              >
                {type}
              </button>
            ))}
      ...
      <div className="flex flex-col items-end gap-4 pointer-events-auto">
        <div className="hud-border bg-hud-bg p-3 rounded-sm text-right backdrop-blur-md">
          <div className="text-[9px] uppercase opacity-50 text-hud-blue/50">OneID Authenticated</div>
          <div className="text-xs font-bold tracking-tighter text-hud-blue">{stats.oneId}</div>
        ...
        <div className="hud-border bg-hud-bg p-4 rounded-sm min-w-[150px] backdrop-blur-md">
          <div className="text-[9px] uppercase opacity-50 mb-1 text-hud-blue/50">Ammo Reserves</div>
          <div className="flex items-baseline gap-2 text-hud-blue">
        ...
        <div className="hud-border bg-hud-bg p-4 rounded-sm min-w-[150px] backdrop-blur-md">
          <div className="text-[9px] uppercase opacity-50 mb-1 text-hud-blue/50">Vaccine Fragments</div>
          <div className="flex items-baseline gap-2">
            <span className={`text-3xl font-bold tracking-tighter ${stats.fragments >= 10 ? 'text-green-500' : 'text-hud-blue'}`}>
              {stats.fragments}
            </span>
            <span className="text-[10px] opacity-50 uppercase text-hud-blue/50">/ 10</span>
          </div>
