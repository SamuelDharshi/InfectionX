...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  // ── Auto-Select State ──────────────────────────────────────────────────────
  useEffect(() => {
    if (inventory.ammo.length > 0 && !selectedAmmoId) {
      setSelectedAmmoId(inventory.ammo[0].objectId);
    }
  }, [inventory.ammo, selectedAmmoId]);

  const refreshVirusState = useCallback(async () => {
    const VIRUS_STATE_ID = process.env.NEXT_PUBLIC_VIRUS_STATE_ID;
    const client = getOneWalletClient();
    if (!client || !VIRUS_STATE_ID) return;

    try {
      const res: any = await client.getOwnedObjects({ // Actually it's a shared object, but in OneWallet bridge we use a general fetch
        owner: VIRUS_STATE_ID,
      });
      // @ts-expect-error - bridge types
      const data = res.data?.[0]?.content?.fields || res.data?.[0]?.content;
      if (data && data.infection_rate != null) {
        setInfectionRate(Number(data.infection_rate));
      }
    } catch (err) {
      console.warn("[WalletContext] Failed to fetch virus state:", err);
    }
  }, []);
  ...
  const refreshInventory = useCallback(async () => {
    ...
    // Also refresh virus state in the bg
    void refreshVirusState();
  ...
  const spawnZombie = useCallback(async () => {
    const client = getOneWalletClient();
    ...
    try {
      await client.moveCall({
        ...
        func: "spawn_zombie_for_hunt",
        args: [
          process.env.NEXT_PUBLIC_VIRUS_STATE_ID!,
          "0x8", // Standard random object address
        ],
      });
      await refreshZombies();
    } catch (err) {
      console.error("[WalletContext] Failed to spawn zombie:", err);
      throw err;
    }
  }, [address, refreshZombies]);
